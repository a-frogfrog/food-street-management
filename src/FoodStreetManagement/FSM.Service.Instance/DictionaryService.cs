...
namespace FSM.Service.Instance
{
    ...
    public class DictionaryService : ResponseHelper, IDictionaryService
    {
        ...
        public async Task<ApiResponse> GetDictionaryList()
        {
            ...
            List<GetDictionaryResponseDto> data = new List<GetDictionaryResponseDto>();
        ...
        /// <returns></returns>
        private List<GetDictionaryResponseDto> GetDictionaryChildren(List<Dictionary> query, string dictionaryId)
        {
            var children = query.Where(q => q.ParentId == dictionaryId).ToList();

            List<GetDictionaryResponseDto> response = new List<GetDictionaryResponseDto>();
        ...
        public async Task<ApiResponse> CreateDictionary(CreateDictonaryRequestDto dto)
        {
            ...
            if (string.IsNullOrEmpty(dto.ParentId))
            {
                ...
                dictionary = CreateDictionaryObject(dto, true, serialNumber);
            }
            else
            {
                ...
                dictionary = CreateDictionaryObject(dto, false, serialNumber, level);
            }
        ...
        /// <returns></returns>
        private Dictionary CreateDictionaryObject(CreateDictonaryRequestDto dto, bool isRoot, int serialNumber = 1, int level = 1)
        {
            Dictionary dictionary = new Dictionary()
            {
